...
- **Osiris Scorers**: built-in scorers that are ready to use
...
We introduce **Osiris**, a **state-of-the-art hallucination detection system** developed by researchers at **Judgment Labs** and the **Stanford Artificial Intelligence Laboratory (SAIL)**, under the guidance of Professor Chris Manning.

Osiris leverages cutting-edge techniques—including:
- **Data augmentation**
- **Test-time scaling**
- **Reinforcement learning**

—to achieve **significantly more accurate hallucination detection** than traditional LLM-as-a-judge approaches.

In addition to its superior performance, Osiris is also **substantially more cost-effective** than closed-source LLMs, making it a high-quality and scalable solution for real-world deployment.

<Tip>
We'll be releasing a report soon detailing some of the novel data augmentation techniques we've developed—stay tuned! 
</Tip>

#### Standard Scorers

Here are the standard scorers in `judgeval`, which our designed to evaluate a single component within your workflow, such as a **retriever call**, **tool invocation**, or **LLM generation**.
...
#### Agentic Scorers
Osiris also includes specialized scorers designed to evaluate multiple steps together within your LLM system—rather than evaluating one step at a time. These agentic scorers are especially useful for multi-step workflows, such as agents, chains, or graphs, where the quality of the overall process matters more than individual outputs.
