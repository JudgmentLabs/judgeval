...
```
```typescript rule_with_config.ts
...
```
```typescript config_scheduled.ts
import { NotificationConfig } from 'judgeval';
...
``` 
```typescript practical_example.ts
