...
def generate_schema_name(path: str, method: str, location: str, status_code: str = "") -> str:
    """Generate a schema name from path, method, and location."""
    # Clean up the path to make a valid identifier
    path_parts = path.strip("/").replace("/", "_").replace("-", "_").replace("{", "").replace("}", "")
                    ...
                    if "schema" in json_content:
                        ...
                        if is_extractable_schema(schema):
                            ...
                            if h in schema_by_hash:
                                ...
                                schema_name = generate_schema_name(path, method, "Response", status_code)
                                schemas[schema_name] = schema
                                schema_by_hash[h] = schema_name
        ...
        if schema.get("type") == "array" and "items" in schema:
            ...
            if isinstance(items, dict) and is_extractable_schema(items):
                ...
                if h in schema_by_hash:
                    ...
                    nested_name = f"{parent_name}_{prop_name}_Item" if prop_name else f"{parent_name}_Item"
        ...
        if "properties" in schema:
            for prop, prop_schema in schema["properties"].items():
                if isinstance(prop_schema, dict):
                    schema["properties"][prop] = process_schema(prop_schema, parent_name, prop)
