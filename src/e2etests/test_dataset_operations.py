...
def test_dataset_list_after_creation(client: JudgmentClient, project_name: str, random_name: str):
    ...
    assert len(updated_datasets) == initial_count + 2, f"Expected {initial_count + 2} datasets, got {len(updated_datasets)}"
...
def test_dataset_list_reflects_changes(client: JudgmentClient, project_name: str, random_name: str):
    ...
    assert random_name in dataset_names, f"Dataset {random_name} should be in the list after creation"
    ...
    assert random_name in updated_dataset_names, f"Dataset {random_name} should still be in the list after overwrite"
    
    overwritten_dataset_info = next(d for d in updated_datasets if d.name == random_name)
    assert overwritten_dataset_info.entries == 0, "Dataset should have 0 entries after overwriting with empty list"
