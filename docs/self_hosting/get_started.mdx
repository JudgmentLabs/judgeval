...
2. A Supabase organization that you have admin access to: this will be used to store and retrievedata for the self-hosted Judgment instance.
3. An available email address and the corresponding *app password* (not the same as your *email password*) for the email address (e.g. no-reply@organization.com). This email address will be used to send email invitations to users on the self-hosted instance.
...
<Warning>
Make sure the Judgment CLI is installed before proceeding. Please refer to [Installation](/docs/judgment_cli/installation) for more information.
</Warning>
