...
The notification system works in conjunction with the rules system to:
...
| `message_template` | string | A template string for customizing the notification message |
| `email_addresses` | list of strings | Email addresses to send notifications to |
| `send_at` | integer (Unix timestamp) | Schedule notifications for a specific time |
...
You can schedule notifications to be sent at a specific time using the `send_at` parameter:
