...
3. An available email address and the corresponding app password for the email address (e.g. no-reply@organization.com). This email address will be used to send email invitations to users on the self-hosted instance.
...
<Tip>
Make sure to follow the prerequisites section carefully before starting the deployment process. This includes installing and configuring AWS CLI and Terraform.
</Tip>
...
Visit the url `https://app.judgmentlabs.ai/login/self-host?api_url={self_hosted_judgment_api_url}` to login to your self-hosted instance. Your self-hosted Judgment API URL will be whitelisted when we review your request from Part 2.

You should be able to log in with the root user you created during the setup process. 
