...
<CodeGroup>
```python wrap_example.py
from openai import OpenAI
from judgeval.tracer import wrap

client = wrap(OpenAI())
```
```typescript wrap_example.ts
...
```
```typescript together_example.ts
...
async function main() {
    ...
    try {
        ...
        // Manually interact with the trace client
