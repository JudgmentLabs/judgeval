...
With deep tracing enabled, you only need to observe top-level functions, and all nested function calls will be automatically traced. This significantly reduces the amount of instrumentation needed in your code.
...
Here's an example using deep tracing:

<CodeGroup>
```Python Python
# Initialize tracer with deep tracing enabled
...
### Deep Tracing Configuration

You can enable deep tracing globally when initializing the tracer:
...
Or you can enable it for specific functions:

<CodeGroup>
```Python Python
# Enable deep tracing for a specific function
@judgment.observe(deep_tracing=True)
def my_function():
    # All function calls inside will be traced
...
If you prefer more granular control or are not using deep tracing, you can still observe each function individually:
