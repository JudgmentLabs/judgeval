...
```
```Typescript push_pull_judgment.ts
...
// Saving (Pushing)
// Use await in an async function context
await client.pushDataset(dataset, alias, projectName, true); // overwrite=true
